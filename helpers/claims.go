...
func (c CustomClaims) Valid() error {
	...
	if !c.VerifyExpiresAt(now, false) {
		...
		vErr.Errors |= j.ValidationErrorExpired
	}

	if !c.VerifyIssuedAt(now, false) {
		vErr.Inner = fmt.Errorf("Token used before issued")
		vErr.Errors |= j.ValidationErrorIssuedAt
	}

	if !c.VerifyNotBefore(now, false) {
		vErr.Inner = fmt.Errorf("token is malformed")
		vErr.Errors |= j.ValidationErrorNotValidYet
	}

	if c.CompanyName == "" {
		vErr.Inner = fmt.Errorf("token is malformed")

		vErr.Errors |= j.ValidationErrorMalformed
	}
	if c.PhoneNumber == "" {
		vErr.Inner = fmt.Errorf("token is malformed")
